local hsluv = require("aylin.hsluv")
...
-- local ns = vim.api.nvim_create_namespace("aylin")
...
function util.debug(colors)
  colors = colors or require("aylin.colors")
  -- Dump unused colors
...
function util.autocmds(config)
  ...
  if config.dev then
    vim.cmd([[  autocmd BufWritePost */lua/aylin/** nested colorscheme aylin]])
  end
